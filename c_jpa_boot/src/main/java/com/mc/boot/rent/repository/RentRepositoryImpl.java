...
public class RentRepositoryImpl implements RentRepositoryExtension{
	...
	public List<RentDto> testProjections() {
		//대출자 아이디가 test인 모든 대출건의, 대출건 제목과 대출변호인 조회
		List<RentDto> rents =
				query.select(Projections.fields(RentDto.class, rent.title, rent.rmIdx))
				.from(rent)
	...
	public List<Tuple> testThetaJoin() {
		...
		//cross join이 발생
			List<Tuple> tuples =
					query.select(rentBook.book.title, member.userId)
					.from(rentBook, member)
					.where(member.userId.eq("test"))
					.fetch();
		return tuples;
	}
